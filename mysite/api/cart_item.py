...
async def create_cart_item(cart_data: CartItemSchema, db: Session = Depends(get_db)):
    cart_item_db = CartItem(**cart_data.dict())
    db.add(cart_item_db)
    db.commit()
...
@cart_item_router.put('/update', response_model=CartItemSchema)
async def update_cart_item(cart_item_id:int, cart_data: CartItemSchema, db: Session = Depends(get_db)):
    cart_item_db = db.query(CartItem).filter(CartItem.id == cart_item_id).first()
    if not cart_item_db:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail='Mynday cart jok')
    cart_item_db.product_id == cart_data.product_id
    db.commit()
    db.refresh(cart_item_db)
    return cart_item_db

@cart_item_router.delete('/delete',response_model=dict)
async def delete_cart_item(cart_id: int, db: Session = Depends(get_db)):
